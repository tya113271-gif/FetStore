...
function openProductModal(p) {
  ...
  if (buyBtn) {
    if (p.checkoutUrl) {
      ...
      buyBtn.onclick = () => {
        toast('جاري تحويلك لصفحة الشراء والإضافة للسلة 🛒...');
      };
    } else {
      ...
      buyBtn.onclick = () => {
        if (discLink.startsWith('#')) {
          $('detailModal').close();
        } else {
          toast('جاري توجيهك إلى ديسكورد للشراء وفتح تذكرة 🛒');
        }
      };
    }
  }
